...
MySQL 권한 설정 : 
```
]# mysql -uroot -p -e 'create database labs';
]# mysql -uroot -p -e "grant all on labs.* to labs@127.0.0.1 identified by 'labs'" 
]# mysql -uroot -p -e "grant all on labs.* to labs@localhost identified by 'labs'" 
```
...
# cp /jboss/labs/modules/mysql/main/mysql-connector-java-5.1.39-bin.jar .
...
##module.xml
```
cp /jboss/labs/modules/mysql/main/module.xml .
...
/] /subsystem=datasources/jdbc-driver=mysql5:add(driver-name=mysql5, driver-module-name="com.mysql", driver-class-name=com.mysql.jdbc.Driver)
{"outcome" => "success"}
```
##Driver 목록확인
```
...
<subsystem xmlns="urn:jboss:domain:datasources:1.2">
    <datasources>
        <datasource jndi-name="java:/mysql" pool-name="mysql5" enabled="true">
            <connection-url>jdbc:mysql://localhost:3306/labs</connection-url>
...
jboss-cli.sh
```
/subsystem=datasources/data-source=mysql5:add(enabled=true,connection-url=jdbc:mysql://localhost:3306/labs,jndi-name=java:/mysql,driver-name=mysql5)
...
/subsystem=datasources/data-source=mysql5:write-attribute(name=pool-prefill,value=true)
/subsystem=datasources/data-source=mysql5:write-attribute(name=pool-use-strict-min,value=true)
/subsystem=datasources/data-source=mysql5:write-attribute(name=user-name,value=labs)
/subsystem=datasources/data-source=mysql5:write-attribute(name=password,value=labs)
